...
def create_realistic_handwriting():
    ...
    font_paths = [
        ...
        r"C:\Windows\Fonts\HWFS.TTF",  # 华文行楷
    ...
    # 添加淡淡的横线（信纸效果）
    line_color = (235, 230, 220)
    for y in range(100, height - 50, 45):
        # 线条略微弯曲
        points = []
        for x in range(80, width - 60, 5):
            offset = int(math.sin(x * 0.01) * 1.5)
    ...
    for y in range(50, height - 30):
        offset = int(math.sin(y * 0.015) * 1.5)
        red_color = (220, 180, 180)
        draw.point((80 + offset, y), fill=red_color)

    def draw_handwritten_char(x, y, char, font, color, size_variation=0.15):
        """绘制单个手写字符，带随机变化"""
        # 随机大小变化
        ...
        # 缩放
        if size_factor != 1:
            new_size = (int(char_img.width * size_factor), int(char_img.height * size_factor))
            char_img = char_img.resize(new_size, Image.Resampling.LANCZOS)
        
        # 粘贴到主图
        paste_x = int(x + offset_x - char_img.width // 2 + 15)
        paste_y = int(y + offset_y - char_img.height // 2 + 15)
        
        img.paste(char_img, (paste_x, paste_y), char_img)

    def draw_handwritten_line(x, y, text, font, color, char_spacing=28):
        """绘制一行手写文字"""
        current_x = x
        for char in text:
            # 每个字符的间距有随机变化
            spacing = char_spacing + random.uniform(-3, 3)
            draw_handwritten_char(current_x, y, char, font, color)
            current_x += spacing
    ...
    # 标题
    title = "\u611f\u8c22\u4fe1"
    title_width = len(title) * 45
    draw_handwritten_line((width - title_width) // 2, y_position, title, font_large, text_color, char_spacing=48)
    y_position += 80

    # 称呼
    draw_handwritten_line(120, y_position, "\u4eb2\u7231\u7684\u90ed\u5955\u9633\uff1a", font_normal, text_color)
    y_position += 55

    draw_handwritten_line(120, y_position, "\u4f60\u597d\uff01", font_normal, text_color)
    y_position += 60
    ...
    for para in paragraphs:
        ...
        for line in lines:
            # 每行整体有轻微倾斜
            line_offset = random.uniform(-5, 5)
            draw_handwritten_line(140 + line_offset, y_position, "\u3000\u3000" + line, font_normal, text_color)
            y_position += 48
        y_position += 15
    ...
    for line in signature_lines:
        ...
        line_width = len(line) * 28
        offset = random.uniform(-8, 8)
        draw_handwritten_line(width - line_width - 100 + offset, y_position, line, font_normal, text_color)
        y_position += 48

    # 轻微模糊效果模拟手写
    img = img.filter(ImageFilter.SMOOTH)

    output_path = r"D:\AI\thank_you_realistic.png"
