...
def create_handwritten_letter():
    ...
    for i in range(0, height, 3):
        offset = int(math.sin(i * 0.02) * 2)
        draw.line([(70 + offset, i), (70 + offset, i + 3)], fill=line_color, width=2)

    def draw_wobbly_text(x, y, text, font, color, wobble=3):
        """绘制歪歪扭扭的文字"""
        for i, char in enumerate(text):
            # 随机偏移
            offset_x = random.randint(-wobble, wobble)
            offset_y = random.randint(-wobble, wobble)
            # 随机旋转效果通过位置偏移模拟
            char_x = x + i * 24 + offset_x
            char_y = y + offset_y
            draw.text((char_x, char_y), char, fill=color, font=font)

    def draw_wobbly_text_large(x, y, text, font, color, wobble=2):
        """绘制大号歪歪扭扭文字"""
        for i, char in enumerate(text):
            offset_x = random.randint(-wobble, wobble)
            offset_y = random.randint(-wobble, wobble)
            char_x = x + i * 38 + offset_x
            char_y = y + offset_y
            draw.text((char_x, char_y), char, fill=color, font=font)
    ...
    draw_wobbly_text_large((width - title_width) // 2, y_position, title, font_large, text_color, wobble=2)
    y_position += 70

    # 称呼
    draw_wobbly_text(90, y_position, "\u4eb2\u7231\u7684\u90ed\u5955\u9633\uff1a", font_normal, text_color)
    y_position += 50

    draw_wobbly_text(90, y_position, "\u4f60\u597d\uff01", font_normal, text_color)
    y_position += 55
    ...
    for para in paragraphs:
        ...
        for line in lines:
            # 每行整体有轻微倾斜
            line_offset = random.randint(-3, 3)
            draw_wobbly_text(110 + line_offset, y_position, "\u3000\u3000" + line, font_normal, text_color, wobble=2)
            y_position += 42
        y_position += 12
    ...
    for line in signature_lines:
        ...
        draw_wobbly_text(width - line_width - 90 + offset, y_position, line, font_normal, text_color, wobble=3)
        y_position += 42

    output_path = r"D:\AI\thank_you_letter_handwritten.png"
